...
typedef struct nfapi_vnf_config
{
	...
	#ifdef NFAPI
	/*! A callback to handle the nr phy config response
	 *  \param msg A pointer to an response message
	 */
	uint8_t (*intgr_nr_config_resp)(uint32_t msgLen, void *msg);

	/*! A callback to handle the nr phy start response
	 *  \param msg A pointer to an response message
	 */
	uint8_t (*intgr_nr_start_resp)(uint32_t msgLen, void *msg);
	#endif
	/* ============================================ */
